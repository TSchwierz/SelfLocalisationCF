...
def drift_vector(position, drift_coefficient):
     ...
     return drift_direction * drift_coefficient * distance
...
def check_for_boundaries(boundaries, position, move_towards):
    ...
    move_towards[out_of_bounds] *= -1  # reverse if out-of-bounds
...
modi = 2 #(s) time between two movement commands
size = 0.1 # magnitude of movement vector
bias_d = 0 # (-1, 1) randomness of movement direction
bias_y = -0.95 # (-1, 1) randomness of rotation angle
drift_coef = 0.95  # adjust new directions to point more towards the origin
choices = ['translation', 'rotation', 'altitude']
p_choices = [0.8, 0.2, 0.0]
boundaries = np.array([[-4.6, 4.6],[-4.6, 4.6],[0, 4]]) # [[-x, +x][-y, +y][z=0, z=max]]
...
# Main loop:
while robot.step(timestep) != -1 and elapsed_time < 360*2: # max time in hours #:
